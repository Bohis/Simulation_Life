	...
	class Bot : ICloneable {
		...
		public Bot(Field link , int i = 0 , int j = 0 , int HP = 100 , int Energy = 100 , sbyte MaxTemp = 50 , sbyte MinTemp = -50 , int Damage = 60 , NeuralNetwork Brain = null , int TrainingCoof = 3 , bool TriggerClone = false) {
			...
			if (TriggerClone == false) {
				Link.LIST_BOT.Add( HashName , this );
		...
		void Generation(string StringMove) {
			try {
				...
				if (StringMove.Split( '|' )[ 0 ] == "1") {
					Link.SetBot( new Bot( Link , (int)IG - 1 , (int)JG , HP_GET + ForTempHp.Next( -3 , 3 ) , 300 , (sbyte)( TempRange.MaxTemp + ForTempHp.Next( -3 , 3 ) ) , (sbyte)( TempRange.MinTemp + ForTempHp.Next( -3 , 3 ) ) , DamageThis + ForTempHp.Next( -3 , 3 ) , BRAIN ) , (int)IG - 1 , (int)JG );
					ENERGY -= 150;
					Info.Generation++;
				}
				else
					if (StringMove.Split( '|' )[ 1 ] == "1") {
					Link.SetBot( new Bot( Link , (int)IG , (int)JG - 1 , HP_GET + ForTempHp.Next( -3 , 3 ) , 300 , (sbyte)( TempRange.MaxTemp + ForTempHp.Next( -3 , 3 ) ) , (sbyte)( TempRange.MinTemp + ForTempHp.Next( -3 , 3 ) ) , DamageThis + ForTempHp.Next( -3 , 3 ) , BRAIN ) , (int)IG , (int)JG - 1 );
					ENERGY -= 150;
					Info.Generation++;
				}
				else
		...
		void KillOtherBot(string StringMove) {
			...
			if (DamageBot() >= Enemy.DamageBot()) {
				...
				Link.LIST_BOT.Remove( Enemy.HashName );
		...
		/// <summary>
		/// Доступ к полю статистиики жизни бота
		/// </summary>
